...
**The answer is the user's.** There is no tool that takes one. `decide` asks them and returns what they picked, one decision per call — never a list, never a summary followed by a single confirmation. This host has no way for a tool to put a question on screen, so `decide` will say it cannot ask rather than guessing. When it does, you stop: put the one question to the user in this conversation yourself — the options in full, the reason for each, and what each costs later — and wait for them to answer. `sober decide <id> <option>` on the command line is what records what they say. Never answer on their behalf, and never treat a reply you did not get as one. Do not pick for them, and do not treat their earlier remarks as an answer.
