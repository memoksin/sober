...
Read what the user asked for, in their own words, from this conversation. That is what they want built.
...
4. Decompose into nodes. A node is one piece of work one agent can finish and a human can review in one sitting. For each: a title, a description someone who did not plan it can act on, the file globs it is expected to touch, and a **name** — a handle, two to four words, like `run-panel`, `log-wire`, `readme-guide`. The name must be distinct from its siblings **on the board**: a category word three nodes could share is worse than no name, so look at what is already there before picking one. It is not a compressed restatement of the title — the title stays the sentence; the name is what a person says out loud when they mean this node. Draw the edges: what must land before what.
...
5. Open a decision **only** where the answer changes the shape of more than one node — where state lives, where a module boundary falls, how data flows, how errors are handled. Those four are the categories SOBER ships; there is no fifth. A question with one obvious answer is not a decision, it is a detail, and it goes in the node's description.
   ...
   **Before writing a new decision, check whether the board already has one that covers the same question.** Call `decisions` with `all: true` — it lists answered decisions too, each with its chosen option and provenance. If one already on the board, open or answered, covers the same question, put its id in the relevant nodes' `decisions` lists instead of proposing a new one. Derive a new decision only when none matches: SOBER binds the nodes that need an answer to the decision that gives it, and a second decision for a question already settled just means the next plan has to reconcile two answers instead of one.
   ...
   **A question this repository has already answered is still asked, but not from scratch.** Before you write the options, read what the project has already settled: `docs/adr/` and anything else ADR-shaped, `CLAUDE.md` or `AGENTS.md`, the dependencies in `package.json` or whatever manifest is here, and the directory layout and the imports between directories. Where that settles one of the four categories, write the choice this project already made as an option, put its id in `suggested`, and set `derived` to where you read it — a path, a file, "the import graph". Write the other options anyway: a choice with nothing beside it is not a choice, and a project changes its mind.
   ...
   `derived` is not an answer and does not skip anything. The question still goes to the user through `the **decide** skill`, one decision at a time — Before you ask, paste the block the tool returned into this conversation, whole — never a pointer to the board, the dashboard or the CLI. — and the record then says the answer was read off the code and confirmed rather than arrived at. Never set it to a guess: if you are inferring what somebody would probably have chosen rather than reading what they did choose, leave it empty.
...
Do not answer a decision, do not write a brief, and do not run anything. The next step is the user's: `the **decide** skill`.
