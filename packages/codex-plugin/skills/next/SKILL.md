...
2. **Approve.** Show the user the whole approach and every acceptance criterion. Before you ask, paste the block the tool returned into this conversation, whole — never a pointer to the board, the dashboard or the CLI. Then ask a yes or a no with this host's own question tool. Call `approve` with `confirmed: true` only on an explicit yes; on a no, do nothing. You cannot approve on their behalf, and nothing runs without it.
