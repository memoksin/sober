...
1. **Brief.** `/sober:brief` is this step on its own, for when the brief is all that is wanted. Here: read the node and the repository, and write the approach with `write_brief`: how you would do it, in this codebase, naming real files and, inside the approach, the files and line ranges to change (e.g. "Where: packages/core/src/x.ts:120–180") — so the dispatched agent does not have to search for them. With it, the acceptance list — each entry a command that can actually be run here and what passing it proves — and `plain`, the same brief in everyday words for someone with no repository knowledge: what changes, why, how success is checked, and any meaningful risk.
