...
2. **Approve.** After `write_brief`, explain in plain language what will change, why, how success will be checked, and any meaningful trade-off or risk. Keep this faithful to the written brief, but leave its technical details and commands in the dashboard for anyone who wants them. Ask a yes or a no about the explanation: {{askYes}} Say whether yes also starts the node when it becomes ready, according to the `queue` argument or the board's `dispatch.queueByDefault`. Call `approve` with `confirmed: true` only on an explicit yes; on a no, do nothing.
