...
Read the board (`board`). Take {{args}} if it names a node, otherwise the first on **needs-brief**.

A node that is `held` or `blocked` does not get a brief yet, and saying so is the answer: a brief written against an unanswered decision is a guess the whole node inherits. Point at `{{decide}}` and stop.
