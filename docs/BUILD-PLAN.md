...
- `core` exports > 107 (ADR 0028, raised for M2 by ADR 0029, for the run of linked nodes by ADR 0050 and for the distribution by ADR 0051 — the number moves with the number of consumers, by ADR; the surface snapshot above is the guard that actually catches growth. It read 100 here while the test read 101, which ADR 0050 found and corrected).
...
| **3** — `cli` published | ~~npm **provenance** and **trusted publishing**~~ — **configured** in phase 3 session 5 and **reached** by ADR 0053: `.github/workflows/release.yml` has carried `id-token: write`, `NPM_CONFIG_PROVENANCE` and an npm new enough to mint its own credential since then, so no publish token is stored in the repository (REVIEW-2026-08-29 §1.2/2, ADR 0007) — but until 0053 the publish was pnpm's rather than npm's, so `0.0.0` and `0.1.0` went out unattested. The first version with an attestation is the one after 0.1.0. | Provenance is three lines _inside a release workflow_, and nothing is published before M1. Writing the workflow early means maintaining a workflow that publishes nothing. | ~45 min with the release workflow |
| **4** — contributors | `.github/CODEOWNERS` narrowed past `* @memoksin`: an explicit entry for every barrel (`**/index.ts`) and for `docs/`, which turns §5's "barrel files are yours" from a habit into a check (REVIEW-2026-08-29 §1.2/3). **And**: the admin bypass narrows or is removed (ADR 0026). | With one contributor, `*` already covers every path and the bypass exists precisely because a solo repository cannot satisfy a code-owner review. Both only become real when a second person pushes, and as of 2026-09-08 none is expected for some time, so this row waits rather than being scheduled. | ~15 min |
